...
#include "valent-plugin-private.h"
 ...
 * instances. It is used to change and store the enabled state of an extension
 * as well as constructing instances.
 *
 * Each plugin object is associated with a [class@Valent.DataSource], which
 * isolates the cache, data and settings for the extension.
 *
 * Since: 1.0
 */
 ...
 * @plugin_domain: (nullable): a context for an extension
 *
 * Get a GSettings path for @plugin_info in @plugin_domain.
 *
 * If @plugin_domain is %NULL, the path will represent the plugin itself,
                                 ...
                                 const char     *plugin_domain)
{
  ...
  g_assert (plugin_domain != NULL || *plugin_domain != '\0');

  return g_strdup_printf ("/ca/andyholmes/valent/%s/plugin/%s/",
                          plugin_domain,
...
valent_plugin_constructed (GObject *object)
{
  ...
  ValentDataSource *data_source = NULL;
  g_autoptr (GSettingsSchema) schema = NULL;
  ...
  g_assert (self->plugin_domain != NULL && *self->plugin_domain != '\0');

  if (VALENT_IS_DATA_SOURCE (self->source))
    {
      data_source = VALENT_DATA_SOURCE (self->source);
    }
  else
    {
      data_source = valent_resource_get_ancestor (VALENT_RESOURCE (self->source),
  ...
  self->settings = valent_settings_new_full (data_source, schema, path);
  valent_settings_bind (self->settings, "enabled",
                        self,           "enabled",
                        G_SETTINGS_BIND_DEFAULT);
  self->enabled = valent_settings_get_boolean (self->settings, "enabled");
}
...
valent_plugin_class_init (ValentPluginClass *klass)
{
   ...
   *
   * Plugins are loaded automatically and extensions enabled by default, so
  ...
  /**
   * ValentPlugin:plugin-info: (getter get_plugin_info)
   *
   * The [class@Peas.PluginInfo] for the plugin.
   *
   * Since: 1.0
   */
  ...
  /**
   * ValentPlugin:source: (getter get_source)
   *
   * The [class@Valent.Resource] the plugin extends.
   *
   * Since: 1.0
   */
...
valent_plugin_new (ValentResource *source,
                   ...
                   const char     *plugin_domain)
{
  g_return_val_if_fail (VALENT_IS_RESOURCE (source), NULL);
  g_return_val_if_fail (PEAS_IS_PLUGIN_INFO (plugin_info), NULL);
  g_return_val_if_fail (plugin_domain != NULL && *plugin_domain != '\0', NULL);

  return g_object_new (VALENT_TYPE_PLUGIN,
                       "plugin-domain", plugin_domain,
...
valent_plugin_create_extension (ValentPlugin *plugin)
{
  ...
  ret = peas_engine_create_extension (valent_get_plugin_engine (),
                                      ...
                                      "plugin-domain", plugin->plugin_domain,
                                      NULL);
  g_return_val_if_fail (g_type_is_a (G_OBJECT_TYPE (ret), plugin->plugin_type), NULL);
 ...
 * @plugin: a `ValentPlugin`
 *
 * Get the source [class@Valent.Resource] for @plugin.
 *
