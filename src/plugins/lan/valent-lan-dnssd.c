...
enum {
  DNSSD_SERVER_INVALID,
  DNSSD_SERVER_REGISTERING,
  DNSSD_SERVER_RUNNING,
  DNSSD_SERVER_COLLISION,
  DNSSD_SERVER_FAILURE,
};

enum {
  DNSSD_ENTRY_GROUP_UNCOMMITTED,
  DNSSD_ENTRY_GROUP_REGISTERING,
  DNSSD_ENTRY_GROUP_ESTABLISHED,
  DNSSD_ENTRY_GROUP_COLLISION,
  DNSSD_ENTRY_GROUP_FAILURE,
};
                           ...
                           gpointer         user_data)
{
  ...
  g_object_set_data_full (G_OBJECT (saddress),
                          VALENT_DNSSD_SERVICE_NAME,
                          g_strdup (name),
                          g_free);

  /* NOTE: GListModel::items-changed is emitted in Avahi's GMainContext */
  valent_object_lock (VALENT_OBJECT (self));
  if ((self->state & VALENT_DNSSD_STATE_SYNCED) != 0)
    {
      ...
      for (unsigned int i = 0; i < self->items->len; i++)
        {
          ...
          service_name = g_object_get_data (G_OBJECT (saddress),
                                            VALENT_DNSSD_SERVICE_NAME);
                                 ...
                                 ValentLanDNSSD  *self)
{
  ...
  valent_object_lock (VALENT_OBJECT (self));
  self->state &= ~VALENT_DNSSD_STATE_SYNCING;
  self->state |= VALENT_DNSSD_STATE_SYNCED;
  valent_object_unlock (VALENT_OBJECT (self));
}
                                   ...
                                   ValentLanDNSSD  *self)
{
  ...
  valent_object_lock (VALENT_OBJECT (self));
  g_assert (self->service_browser_path == NULL);
...
_avahi_service_browser_prepare (ValentLanDNSSD *self)
{
  ...
  valent_object_lock (VALENT_OBJECT (self));
  if ((self->state & VALENT_DNSSD_STATE_SYNCING) == 0 &&
      (self->state & VALENT_DNSSD_STATE_SYNCED) == 0)
    {
      GVariant *args = NULL;

      self->state |= VALENT_DNSSD_STATE_SYNCING;

      args = g_variant_new ("(iissu)",
                            -1,                        // interface: AVAHI_IF_UNSPEC
                            -1,                        // protocol: AVAHI_PROTO_UNSPEC
                            VALENT_DNSSD_SERVICE_TYPE, // type
                            "",                        // domain
                            0);                        // AvahiLookupFlags
 ...
 */
static void
_avahi_entry_group_commit_cb (GDBusConnection *connection,
                              GAsyncResult    *result,
                              ValentLanDNSSD  *self)
{
  g_autoptr (GVariant) reply = NULL;
  ...
  valent_object_lock (VALENT_OBJECT (self));
  self->state &= ~VALENT_DNSSD_STATE_REGISTERING;
  self->state |= VALENT_DNSSD_STATE_REGISTERED;
  valent_object_unlock (VALENT_OBJECT (self));
}
...
{
  valent_object_lock (VALENT_OBJECT (self));
  if ((self->state & VALENT_DNSSD_STATE_REGISTERING) != 0 &&
      (self->entry_group_state == DNSSD_ENTRY_GROUP_UNCOMMITTED))
    {
      g_dbus_connection_call (self->connection,
                              AVAHI_DBUS_NAME,
                              ...
                              -1,
                              self->cancellable,
                              (GAsyncReadyCallback)_avahi_entry_group_commit_cb,
                              self);
    }
  valent_object_unlock (VALENT_OBJECT (self));
}
...
_avahi_entry_group_add_service (ValentLanDNSSD *self)
{
  ...
  if ((self->entry_group_path != NULL) &&
      (self->state & VALENT_DNSSD_STATE_REGISTERING) == 0 &&
      (self->state & VALENT_DNSSD_STATE_REGISTERED) == 0)
    {
      GVariant *args = NULL;

      self->state |= VALENT_DNSSD_STATE_REGISTERING;

      args = g_variant_new ("(iiussssq@aay)",
                            -1, // interface: AVAHI_IF_UNSPEC
                            -1, // protocol:  AVAHI_PROTO_UNSPEC
                            64, // flags:     AVAHI_PUBLISH_UPDATE
                            self->name,
                            VALENT_DNSSD_SERVICE_TYPE,
                            "", // domain
                            "", // host
                            self->port,
                            self->txt);
                                  ...
                                  gpointer         user_data)
{
  ...
  switch (self->entry_group_state)
    {
    case DNSSD_ENTRY_GROUP_UNCOMMITTED:
    case DNSSD_ENTRY_GROUP_REGISTERING:
    case DNSSD_ENTRY_GROUP_ESTABLISHED:
      break;

    case DNSSD_ENTRY_GROUP_COLLISION:
    ...
    case DNSSD_ENTRY_GROUP_FAILURE:
                           ...
                           ValentLanDNSSD  *self)
{
  ...
  valent_object_lock (VALENT_OBJECT (self));
  g_assert (self->entry_group_path == NULL);
...
static void
_avahi_entry_group_update_service_txt_cb (GDBusConnection *connection,
                                          GAsyncResult    *result,
                                          ValentLanDNSSD  *self)
{
  g_autoptr (GVariant) reply = NULL;
...
static gboolean
_avahi_entry_group_update_service_txt (ValentLanDNSSD *self)
{
  valent_object_lock (VALENT_OBJECT (self));
  if ((self->state & VALENT_DNSSD_STATE_REGISTERED) != 0 &&
      (self->state & VALENT_DNSSD_STATE_REGISTERING) == 0 )
    {
      GVariant *args = NULL;
      ...
      g_dbus_connection_call (self->connection,
                              ...
                              -1,
                              self->cancellable,
                              (GAsyncReadyCallback)_avahi_entry_group_update_service_txt_cb,
                              self);
    }
  valent_object_unlock (VALENT_OBJECT (self));
                             ...
                             gpointer         user_data)
{
  ...
  switch (self->server_state)
    {
    case DNSSD_SERVER_INVALID:
    case DNSSD_SERVER_REGISTERING:
      break;

    case DNSSD_SERVER_RUNNING:
      _avahi_entry_group_new (self);
      _avahi_service_browser_prepare (self);
      break;

    case DNSSD_SERVER_COLLISION:
      g_warning ("%s(): DNS-SD server collision: %s", G_STRFUNC, error);
      break;

    case DNSSD_SERVER_FAILURE:
  ...
  if (g_set_object (&self->connection, connection))
    {
      ...
      if (self->server_state == DNSSD_SERVER_RUNNING)
        {
          g_main_context_invoke_full (self->context,
                                      G_PRIORITY_DEFAULT,
  ...
  if (self->connection != NULL)
    {
      ...
      self->state = VALENT_DNSSD_STATE_NONE;
      self->server_state = DNSSD_SERVER_INVALID;
      self->entry_group_state = DNSSD_ENTRY_GROUP_FAILURE;
...
static gboolean
_avahi_service_start (ValentLanDNSSD *self)
{
  valent_object_lock (VALENT_OBJECT (self));
...
static gboolean
_avahi_service_shutdown (ValentLanDNSSD *self)
{
  valent_object_lock (VALENT_OBJECT (self));
  if (self->connection != NULL)
    {
  ...
  if (self->connection != NULL)
    {
      g_main_context_invoke_full (self->context,
                                  G_PRIORITY_DEFAULT,
                                  G_SOURCE_FUNC (_avahi_entry_group_update_service_txt),
                                  g_object_ref (self),
                                  g_object_unref);
    }
}
...
valent_lan_dnssd_dispose (GObject *object)
{
  ...
  G_OBJECT_CLASS (valent_lan_dnssd_parent_class)->dispose (object);
}
...
valent_lan_dnssd_class_init (ValentLanDNSSDClass *klass)
{
  ...
  object_class->dispose = valent_lan_dnssd_dispose;
                         ...
                         GMainContext    *context)
{
  ...
  g_main_context_invoke_full (self->context,
                              G_PRIORITY_DEFAULT,
                              G_SOURCE_FUNC (_avahi_service_start),
                              g_object_ref (self),
                              g_object_unref);
  valent_object_unlock (VALENT_OBJECT (self));
}
