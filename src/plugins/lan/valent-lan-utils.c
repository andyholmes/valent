...
static void
g_tls_connection_handshake_cb (GTlsConnection *connection,
                               GAsyncResult   *result,
                               gpointer        user_data)
{
  g_autoptr (GTask) task = G_TASK (g_steal_pointer (&user_data));
  g_autoptr (GTlsCertificate) ca_certificate = NULL;
  GTlsCertificate *trusted = NULL;
   ...
   * TODO: this should be handled by a managed cache object
   */
  trusted = g_task_get_task_data (task);
  if (trusted != NULL)
    {
      ca_certificate = g_object_ref (trusted);
    }
  ...
  g_task_return_pointer (task, g_object_ref (connection), g_object_unref);
}

/**
 * valent_lan_connection_handshake_async:
 * @connection: a `GSocketConnection`
 * @certificate: a `GTlsCertificate`
 * @trusted: (nullable): a `GTlsCertificate`
 * @is_client: %TRUE for client connection, or %FALSE for a server connection
 ...
 * otherwise a [class@Gio.TlsServerConnection].
 *
 * Call [func@Valent.lan_connection_handshake_finish] to get the result.
 */
void
valent_lan_connection_handshake_async (GSocketConnection   *connection,
                                       GTlsCertificate     *certificate,
                                       GTlsCertificate     *trusted,
                                       gboolean             is_client,
                                       GCancellable        *cancellable,
                                       ...
                                       gpointer             user_data)
{
  ...
  g_autoptr (GTlsConnection) ret = NULL;
  GError *error = NULL;
  ...
  g_task_set_source_tag (task, valent_lan_connection_handshake_async);
  if (trusted != NULL)
    g_task_set_task_data (task, g_object_ref (trusted), g_object_unref);
  ...
  g_signal_connect (G_TLS_CONNECTION (ret),
                    ...
                    G_CALLBACK (valent_lan_connection_accept_certificate_cb),
                    NULL);
  g_tls_connection_handshake_async (G_TLS_CONNECTION (ret),
                                    G_PRIORITY_DEFAULT,
                                    cancellable,
                                    (GAsyncReadyCallback)g_tls_connection_handshake_cb,
                                    g_object_ref (task));
}

/**
 * valent_lan_connection_handshake_finish:
 * @connection: a `GSocketConnection`
 * @result: a `GAsyncResult`
 * @error: (nullable): a `GError`
 *
 * Finish an operation started by [func@Valent.lan_connection_handshake_async].
 *
 * Returns: (transfer full) (nullable): a `GTlsConnection`,
 *   or %NULL with @error set.
 */
GIOStream *
valent_lan_connection_handshake_finish (GSocketConnection  *connection,
                                        GAsyncResult       *result,
                                        GError            **error)
{
  g_assert (G_IS_SOCKET_CONNECTION (connection));
  g_assert (g_task_is_valid (result, connection));
  g_assert (error == NULL || *error == NULL);
