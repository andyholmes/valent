...
valent_message_attachment_finalize (GObject *object)
{
  ...
  g_clear_pointer (&self->preview, g_bytes_unref);
                                        ...
                                        GParamSpec *pspec)
{
  ...
  switch ((ValentMessageAttachmentProperty)prop_id)
    ...
    case PROP_PREVIEW:
      g_value_set_boxed (value, self->preview);
      break;
                                        ...
                                        GParamSpec   *pspec)
{
  ...
  switch ((ValentMessageAttachmentProperty)prop_id)
    ...
    case PROP_PREVIEW:
      valent_message_attachment_set_preview (self, g_value_get_boxed (value));
      break;
...
valent_message_attachment_class_init (ValentMessageAttachmentClass *klass)
{
  ...
  properties [PROP_PREVIEW] =
    g_param_spec_boxed ("preview", NULL, NULL,
                        G_TYPE_BYTES,
                        (G_PARAM_READWRITE |
                         G_PARAM_EXPLICIT_NOTIFY |
                         G_PARAM_STATIC_STRINGS));
...
valent_message_attachment_set_preview (ValentMessageAttachment *attachment,
                                       GBytes                  *preview)
{
  g_autoptr (GBytes) old_preview = NULL;

  g_return_if_fail (VALENT_IS_MESSAGE_ATTACHMENT (attachment));

  if (attachment->preview != preview)
    {
      old_preview = g_steal_pointer (&attachment->preview);
      if (preview != NULL)
        attachment->preview = g_bytes_ref (preview);
