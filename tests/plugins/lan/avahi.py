...
# pylint: disable=duplicate-code,import-error,invalid-name,line-too-long,protected-access
...
def EntryGroupNew(self):
    ...
    entry_group.avahi_id = self.n_eg
    entry_group.avahi_state = 2 # AVAHI_ENTRY_GROUP_ESTABLISHED
...
def Reset(self):
    ...
    for name, service in self.avahi_services.items():
        del avahi_server2.avahi_services[name]
...
def ServiceBrowserPrepare(self, _interface, _protocol, _domain, _type, _flags):
    ...
    service_browser = dbusmock.get_object(object_path)
    service_browser.avahi_id = self.n_sb
...
def Start(self):
    ...
    known_services = dbusmock.get_object('/').avahi_services
