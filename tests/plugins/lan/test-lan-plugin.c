...
#define PEER_IDENTITY_BUFFER_MAX        (8192)
  ...
  if (g_test_subprocess ())
    {
      ...
      if (g_strcmp0 (test_name, TEST_INCOMING_IDENTITY_OVERSIZE) == 0 ||
          g_strcmp0 (test_name, TEST_OUTGOING_IDENTITY_OVERSIZE) == 0)
        {
          ...
          oversize = g_strnfill (PEER_IDENTITY_BUFFER_MAX + 1, '0');
...
#if 0
// FIXME: fix cancellation while blocking the cancellable's thread
